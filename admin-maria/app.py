...
def init_db():
    ...
    if not cur.fetchone()[0]:
        ...
        # Assuming profesor1 is the one causing the issue in logs (id=4 after admin=1, profesor1=2, plus potentially other users created earlier)
        # We need to insert a corresponding entry into the 'profesores' table
        profesor1_user_id = cur.lastrowid # Get the ID of the newly inserted profesor1
        cur.execute('''
        ...
        if request.method == 'GET':
            row = conn.execute('''
                SELECT
                    ...
                    u.documento,
                    u.correo,
                    u.contrasena, -- Be cautious with sending passwords in real applications
                    u.rol,
                    u.fecha_registro AS fecha_de_registro,
            ...
            if row:
                ...
                profesor_data['fecha_de_registro'] = datetime.strptime(profesor_data['fecha_de_registro'], '%Y-%m-%d %H:%M:%S').isoformat() if profesor_data['fecha_de_registro'] else None

                # Split nombre into nombre and apellido if it's a full name
                if ' ' in profesor_data['nombre']:
                    first_space_index = profesor_data['nombre'].find(' ')
                    profesor_data['nombre_first'] = profesor_data['nombre'][:first_space_index]
                    profesor_data['apellido'] = profesor_data['nombre'][first_space_index:].strip()
                else:
                    profesor_data['nombre_first'] = profesor_data['nombre']
                    profesor_data['apellido'] = '' # Or handle as appropriate if no last name
            ...
            if 'estado' in data:
                ...
                # Assuming 'nombre' and 'apellido' are provided separately in the update payload
                nombre_completo = f"{data['nombre']} {data['apellido']}"
                conn.execute('''
                  UPDATE usuarios
                  SET nombre=?, nombre_usuario=?, correo=?
                  WHERE id=?
                ''', (nombre_completo, data['nombre_usuario'], data['correo'], profesor_id))
                conn.execute('''
                  UPDATE profesores
            ...
            # This is a soft delete by setting activo to 0
